...
////////////////////
router.post("/signinfp", signInWithFp);
...
function registerHandler(req, res) {
    ...
    if (!req.user) {
        ...
        return jwt.verify(username, process.env.SECRET, async function(
            err,
            decoded,
        ) {
...
async function signInWithFp(req, res) {
    //         console.log('yaboooooyaaaa', req.body.user_name);
    let dataRecord = await User.readUser(req.body.user_name);
    console.log('useeeeeeeeeeer', dataRecord);
    if (dataRecord[0]) {
        let token = users.getToken(dataRecord[0])
        let day = 86400000;
        res.cookie('remember token', token, {
            expires: new Date(Date.now() + day),
            httpOnly: true,
        });
        console.log('this is the toooooooooooooken', token);
        res.send(token);
    } else {
...
async function profilePageHandler(req, res) {
    ...
    await User.read(username).then((data) => {
        let userInfo = {
...
function addpostsHandler(req, res) {
    ...
    postmodule.create(newpost).then((data) => {
...
function editpostsHandler(req, res) {
    ...
    User.read(username).then((data) => {
        postmodule.readById(idpost).then((postdata) => {
            if (postdata[0].user_name === data.user_name) {
                postmodule.update(idpost, newpost).then((data) => {
                    res.json(data);
                });
            } else {
    ...
    User.read(username).then((data) => {
        postmodule.readById(idpost).then((postdata) => {
            if (
                ...
                postdata[0].user_name === data.user_name
            ) {
                postmodule.delete(idpost).then((data) => {
...
function otherUserProfileHandler(req, res) {
    ...
    User.read(username).then((otherUser) => {
...
function addArticleUser(req, res) {
    ...
    User.articleByUser(id1, id2).then((data) => {
...
function deleteArticles(req, res) {
    let id1 = req.user.user_name;
    let id2 = req.params.idarticle;
    User.deleteArticle(id1, id2).then((data) => {
        res.redirect("/profile");
    });
}
